...
Sculptor::Sculptor(int _nx, int _ny, int _nz){
 //Condicionando o número de linhas, colunas e profundidade da matriz
    nl=_nx; nc=_ny; np=_nz;
    //Condição para que não possa haver nl, nc ou np menor do que zero. Caso aconteça, todos nl, ne e np serão iguais a zero
    if(nl<=0||nc<=0||np<=0){
        nl=nc=np=0;
    }
    ...
    //O programa está percorrendo a matriz
    for(int i=1;i<np;i++){
        v[i]=v[i-1]+nc;
    }
...
void Sculptor::setColor(float _r, float _g, float _b, float alpha){
    //Verificação das cores utilizadas. Está condição é para garantir que o usuário poderá modificar a cor no intervalo de zero à um.
    if((_r<=1||_r>=0)||(_g<=1||_g>=0)||(_b<=1||_b>=0)||(alpha<=1||alpha>=0)){
        r=_r;g=_g;b=_b;a=alpha;
    }
    //Caso contrário, será invalidado.
    else {
...
void Sculptor::writeOFF(string filename){
    ...
    int aux=0;
    //Abrindo arquivo.
    fout2.open("C:/Users/joser/OneDrive/Imagens/untitled3"+filename+".off");
...
void Sculptor::writeVECT(string filename){
    ...
    int cont=0;
    //Abrindo arquivo.
    fout.open("C:/Users/joser/OneDrive/Imagens/untitled3"+filename+".vect");
    //Condição para determinar se o arquivo foi aberto.
    if(fout.is_open()){
